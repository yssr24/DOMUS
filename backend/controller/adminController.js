...
// List files under "files/" collection
exports.getFiles = async (_req, res) => {
  try {
    const db = admin.firestore()
    
    // Get all files from Firestore
    const filesSnap = await db.collection('files').orderBy('createdAt', 'desc').get()
    ...
    for (const pid of projectIds) {
      try {
        ...
        if (pDoc.exists) {
          ...
          projectMap[pid] = pData.code || pData.title || pid
        }
      } catch (e) {
        projectMap[pid] = pid
      }
    ...
    // Enrich files with project code
    const enrichedFiles = files.map(f => ({
      ...f,
      projectCode: projectMap[f.projectId] || '—'
    }))
