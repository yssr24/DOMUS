...
<template>
  <div class="project-settings">
    <header class="settings-header">
      ...
      <span :class="['status-badge', statusKey(project.status)]">{{ formatStatus(project.status) }}</span>
    </header>
          ...
          <div v-for="(group, date) in groupedActivities" :key="date" class="activity-group">
            ...
            <div v-for="activity in group" :key="activity.id" class="activity-item">
              <div class="activity-icon" :class="activity.type">
                <svg v-if="activity.type === 'created_project'" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 5v14M5 12h14"/></svg>
                <svg v-else-if="activity.type === 'file_upload'" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                <svg v-else-if="activity.type === 'task_created'" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M9 11l3 3L22 4"/></svg>
                <svg v-else-if="activity.type === 'submission'" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                <svg v-else-if="activity.type === 'message'" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
      ...
      <div class="main-area">
        <section class="card project-details-card">
          ...
          <div class="details-grid">
            ...
            <div class="detail-item">
              <label>Status</label>
              <p><span :class="['status-pill', statusKey(project.status)]">{{ formatStatus(project.status) }}</span></p>
            </div>
            <div class="detail-item">
        ...
        <div class="two-col">
          ...
          <section class="card staff-card">
            ...
            <div v-if="staffMembers.length === 0" class="empty-state small">
            ...
            <table class="data-table">
              ...
              <tbody>
                <tr v-for="file in files" :key="file.id">
                  ...
                  <td>
                    <a :href="file.fileUrl" target="_blank" class="action-btn view" title="View">
...
function formatDate(dateStr) {
  ...
  const d = typeof dateStr === 'object' && dateStr.seconds ? new Date(dateStr.seconds * 1000) : new Date(dateStr)
...
function getFileIcon(type) {
  const icons = { pdf: '📄', dwg: '📐', jpg: '🖼️', jpeg: '🖼️', png: '🖼️', report: '📊' }
...
.header-info { flex: 1; }
.header-info h1 { margin: 0; font-size: 1.5rem; color: #213547; }
.project-code { font-family: ui-monospace, monospace; color: #666; font-size: 0.9rem; }
...
.spinner {
  ...
  border: 3px solid #f0f0f0; border-top-color: #1976d2;
...
.activity-sidebar h2 {
  ...
  margin: 0 0 16px 0; font-size: 1.1rem; color: #213547;
  padding-bottom: 12px; border-bottom: 1px solid #f0f0f0;
}

.activity-list { display: flex; flex-direction: column; gap: 16px; }
.activity-group { display: flex; flex-direction: column; gap: 8px; }
.activity-date {
...
.activity-icon.task_created { background: #f6eaff; color: #8e24aa; }
.activity-icon.submission { background: #e8ffee; color: #0b7a3b; }
.activity-icon.message { background: #fce4ec; color: #c62828; }

.activity-content { flex: 1; min-width: 0; }
.activity-message { margin: 0; font-size: 0.85rem; color: #213547; word-break: break-word; }
...
.count-badge {
  background: #e3f0fc; color: #1976d2;
...
.detail-item label { font-size: 0.8rem; color: #666; font-weight: 600; text-transform: uppercase; }
.detail-item p { margin: 0; color: #213547; }
...
.client-details h3 { margin: 0; color: #213547; }
.client-email { color: #666; font-size: 0.9rem; margin: 4px 0 0; }
...
.staff-name { font-weight: 600; color: #213547; }
.staff-email { font-size: 0.85rem; color: #666; }
...
.data-table th {
  text-align: left; padding: 12px 10px; font-size: 0.8rem;
  color: #666; text-transform: uppercase; border-bottom: 2px solid #f0f0f0;
}
.data-table td { padding: 12px 10px; border-bottom: 1px solid #f5f5f5; color: #213547; }
...
.action-btn:hover { background: #e3f0fc; color: #1976d2; }
...
.file-link { color: #1976d2; font-size: 0.85rem; text-decoration: none; }
.file-link:hover { text-decoration: underline; }
.no-files { color: #999; font-size: 0.85rem; }
...
@media (max-width: 1100px) {
  ...
  .activity-sidebar { position: static; max-height: none; order: 2; }
}
